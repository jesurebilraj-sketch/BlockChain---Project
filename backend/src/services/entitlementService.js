...
class EntitlementService {
  ...
  async checkEligibilityAndQuota(beneficiaryId, commodity, requestedQty) {
    const ben = await this.getBeneficiary(beneficiaryId);
  ...
  async deductQuota(beneficiaryId, commodity, quantity) {
    const ben = await this.getBeneficiary(beneficiaryId);
    const claimed = { ...(ben.currentMonthClaimed || {}) };
